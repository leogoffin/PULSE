...
P5,mdotf,fab = cc.with_Tout(P4,P0.T0,mdot,delta_hf,T0ab,f=f,eta_cc= eta_ab,pi_cc = pi_ab) # after burner
P6,choked,NPR,A = adiabatic_convergent_nozzle(P5,mdot,f+fab,P0.p) #conv nozzle

Tp,Ts = turbofan_thrust(mdot,0,fab,P0,P6,AP=A)
SFC = mdot*fab/Tp
print(f"Choked ? - {choked}")
print(f"Wet thrust conv : {Tp:.0f} N")
print(f"Wet SFC : {SFC*kgN_to_kghDaN:.3f} kg/(hDaN)")
print(f"Nozzle thoughflow section : {A:.2f} m²\n")

P6b,Ab = adiabatic_convdiv_nozzle(P5,mdot,f+fab,P0.p) #conv-div nozzle

Tpb,Tsb = turbofan_thrust(mdot,0,fab,P0,P6b,AP=Ab)
SFCb = mdot*fab/Tpb

print(f"Wet thrust conv-div : {Tpb:.0f} N")
print(f"Wet SFC : {SFCb*kgN_to_kghDaN:.3f} kg/(hDaN)")
